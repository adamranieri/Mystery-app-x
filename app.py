...
async def get_document(docId: str) -> Document:
...
async def multiply(num1:float, num2:float, amount:int) -> str:
...
async def create_random_geopoints(amount:int) -> list[Coordinate]:
...
async def factorial(num: int, memo = {}) -> int:
